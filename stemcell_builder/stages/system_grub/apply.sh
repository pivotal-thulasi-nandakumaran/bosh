...
if [ -d $chroot/etc/grub.d ] # GRUB 2 on CentOS 7 or Ubuntu
then

  echo "Found grub2; leaving config files where they are"

elif [ -d $chroot/usr/lib/grub ] # classic GRUB on Ubuntu
then

  rsync -a $chroot/usr/lib/grub/x86*/ $chroot/boot/grub/

elif [ -d $chroot/usr/share/grub ] # classic GRUB on CentOS 6.5
then
